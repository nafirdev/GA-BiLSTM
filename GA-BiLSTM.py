...
def initialize_population(population_size, parameter_space):
    ...
    for _ in range(population_size):
        individual = [random.choice(parameter_space[key]) for key in parameter_space.keys()]
...
def mutate(individual, mutation_rate, parameter_space):
    for i in range(len(individual)):
        if random.random() < mutation_rate:
            parameter_name = list(parameter_space.keys())[i]
            individual[i] = random.choice(parameter_space[parameter_name])
    return individual


def evaluate_individual(individual, x_train, y_train, x_val, y_val):
    lstm_units, dropout_rate, batch_size, epochs = individual
...
def optimize_lstm_parameters(x_train, y_train, x_val, y_val, parameter_space, population_size, num_generations):
    ...
    for generation in range(num_generations):
        ...
        for individual in population:
            fitness = evaluate_individual(individual, x_train, y_train, x_val, y_val)
            fitness_scores.append((individual, fitness))
        fitness_scores.sort(key=lambda x: x[1])
        selected_parents = [individual for individual, _ in fitness_scores[:population_size // 2]]
...
best_parameters = optimize_lstm_parameters(xtrain, ytrain, xval, yval, parameter_space, population_size=10, num_generations=7)
